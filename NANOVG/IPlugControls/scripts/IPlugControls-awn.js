/* Declares the IPlugControls Audio Worklet Node */

class IPlugControlsController extends WAMController
{
  constructor (actx, options) {
    ...
    super(actx, "IPlugControls", options);
  }

  static importScripts (actx) {
    var origin = "https://iplug2.github.io/NANOVG/IPlugControls/";

    return new Promise( (resolve) => {
      actx.audioWorklet.addModule(origin + "scripts/IPlugControls-wam.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wam-processor.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/IPlugControls-awp.js").then(() => {
        resolve();
      }) }) });
    })
  }
