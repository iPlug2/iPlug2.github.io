/* Declares the IPlugResponsiveUI Audio Worklet Node */

class IPlugResponsiveUIController extends WAMController
{
  constructor (actx, options) {
    ...
    super(actx, "IPlugResponsiveUI", options);
  }

  static importScripts (actx) {
    var origin = "https://iplug2.github.io/CANVAS/IPlugResponsiveUI/";

    return new Promise( (resolve) => {
      actx.audioWorklet.addModule(origin + "scripts/IPlugResponsiveUI-wam.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/wam-processor.js").then(() => {
      actx.audioWorklet.addModule(origin + "scripts/IPlugResponsiveUI-awp.js").then(() => {
        resolve();
      }) }) });
    })
  }
